...
/* Adds a mapping in page directory PD from user virtual page
   ...
   failed. */
//在指定页表pd中生成一条记录，使upage指向kpage对应的的物理页帧，这样kernal virtual address和user virtual address共用这一物理页帧
//为什么要内核和user virtual address共用这一物理页帧？因为咱们写的pintos的执行过程就是一直在内核态。
...
//根据pd查找用户地址对应的内核地址
void *
pagedir_get_page(uint32_t *pd, const void *uaddr, int size)
{
  uint32_t *pte, *tail_pte;
  if (uaddr == NULL)
  {
    return NULL;
  }
  ...
  // tail_pte = lookup_page(pd + size - 1, uaddr, false);
  if (pte != NULL && (*pte & PTE_P) != 0)
  {
    // printf("kernel addr %8x\n", pte_get_page(*pte) + pg_ofs(uaddr));
...
bool pointer_check_valid(uint32_t *pd, const void *uaddr, int size)
{
  ...
  pte = lookup_page(pd, uaddr, false);
  pte_end = lookup_page(pd, uaddr + size - 1, false);
  if (pte != NULL && (*pte & PTE_P) != 0 && pte_end != NULL && (*pte_end & PTE_P) != 0)
  {
    return true;
  }
  else
